...
public class AjouterMessageController {
    ...
    try{
        MessageService ps = new MessageService();
            ps.ajouter(new Message(Integer.parseInt(tfAjouterMessage.getText()) , dpAjouterMessage.getValue().toString(), taAjouterMessage.getText(),Session.getCurrentUser().getId(),0)) ;
