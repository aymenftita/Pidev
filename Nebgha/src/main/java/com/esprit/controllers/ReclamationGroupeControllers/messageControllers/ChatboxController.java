...
public class ChatboxController {
    ...
    public void initialize() {
        ...
        List<UserGroupe> ug= ugs.ListUserGroupeByIdUser(Session.getCurrentUser().getId());
    ...
    private void displayChatHistory(List<Message> chatHistory) {
        for (Message message : chatHistory) {
            if (us.getUser(message.getUid()) != null) {
                Label messageLabel = new Label(us.getUser(message.getUid()).getNom() + ": " + message.getText());
                ...
                ServiceUtilisateur us = new ServiceUtilisateur();
                typing.setText(us.getUser(Session.getCurrentUser().getId()).getNom().toUpperCase(Locale.ROOT) + " is typing now");
                typing.visibleProperty().bind(messageTextField.textProperty().isNotEmpty());
                if (message.getUid() == Session.getCurrentUser().getId()) {
    ...
    public void handleSendButtonActionI(MouseEvent mouseEvent) throws IOException {
        ...
        if (!messageContent.isEmpty()) {
            if(messageContent.startsWith("info:")){
                ...
                Message newMessage = new Message(selectedGroup.getId_groupe(), String.valueOf(LocalDate.now()), cleanValue, Session.getCurrentUser().getId(), 0);
                ms.ajouter(newMessage);
                Label newM = new Label(us.getUser(newMessage.getUid()).getNom() + " : " + cleanValue + "\n");
                newM.wrapTextProperty();
                ...
                String cleanValue = TextFiltringApi(messageContent);
                Message newMessage = new Message(selectedGroup.getId_groupe(), String.valueOf(LocalDate.now()), cleanValue, Session.getCurrentUser().getId(), 1);
                ms.ajouter(newMessage);
                Label newM = new Label(us.getUser(newMessage.getUid()).getNom() + " : " + cleanValue + "\n");
...
@FXML
    public void SwitchToGroupes(MouseEvent actionEvent) throws IOException {
        SwitchScenesController ss =new SwitchScenesController();
        ActionEvent event = null;
        ss.SwitchScene2(event,"testFlow",messageTextField);
    }
